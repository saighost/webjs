...
var filesBuffers = {};
function sendfile (_fileName, res, found) {
    ...
    if (filesBuffers[_fileName] == undefined) {
        fs.stat(_fileName, function (err, stats) {
            ...
            var needCache = expires.cacheFormat.test(_fileName);
            if (needCache) {
            ...
            fileStream.on('data', function (chunk) {
                res.write(chunk);
                if (needCache) filesBuffers[_fileName].write(chunk.toString('utf8'));
            }).on('end', function () {
                res.end();
            });
        });
...
http.ServerResponse.prototype.sendError = function (statu) {
    ...
    var handler = global.web.lookup('error', String(statu));
    var self = this;
    if (handler) {
 ...
 */
http.ServerResponse.prototype.sendFile = function (fileName, found) {
    if (!found) {
        ...
        if ('/' in _server.handlers) {
            if (new RegExp(_server.handlers.url['/'], 'i').test(fileName)) {
                return sendfile(fileName, this);
            } else if (/\/$/i.test(_server.handlers.url['/']) && !/^\//i.test(fileName)) {
                return sendfile(_server.handlers.url['/'] + fileName, this);
            } else {
                return sendfile(_server.handlers.url['/'] + '/' + fileName, this);
            }
        }
    } else {
        sendfile(fileName, this, true);
    }
    return this;
};
/*
